...
def analemmic_horiz_hour_angle(hour, location, timezone):
    equatorial_angle = equatorial_hour_angle(hour, location, timezone)
    equatorial_angle_from_solar_noon = equatorial_angle - np.pi
...
#                                     theta1=np.rad2deg(analemmic_angle_min),
#                                     theta2=np.rad2deg(analemmic_angle_max)
...
#                                     theta2=135
    ...
    for hour in range(HOUR_LINE_MIN, HOUR_LINE_MAX + 1):
        analemmic_angle = analemmic_horiz_hour_angle(hour, LOCATION, TIMEZONE)
        if LOCATION.latitude < 0:
            # For southern hemisphere, rotate the whole thing around by 180
            # degrees, so "up" is consistently from the sundial viewer's
            # perspective with the sun behind their shoulder.
            analemmic_angle += np.deg2rad(180)
        logging.info("For hour %d, horiz angle %g" % (hour, np.rad2deg(analemmic_angle)))
        line = lines.Line2D([0, np.cos(analemmic_angle)], [0, np.sin(analemmic_angle)])
        ax1.add_line(line)
        hour_text = "%d" % ((hour - 1) % 12 + 1)
        ax1.add_artist(text.Text(np.cos(analemmic_angle) * NUMERAL_OFFSET, np.sin(analemmic_angle) * NUMERAL_OFFSET, hour_text, ha='center', va='center'))
